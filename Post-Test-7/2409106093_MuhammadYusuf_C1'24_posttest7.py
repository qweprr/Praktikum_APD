...
# Fungsi Untuk Cek Oshi
def Cek_Oshi_Ada(nama_Oshi):
    return nama_Oshi in oshis

# Fungsi untuk mendapatkan jumlah Oshi pengguna
def get_user_oshi_count(username):
    return len(akuns[username]["Oshi"])

# Fungsi untuk menampilkan detail Oshi 
def Detail_Oshi(nama_Oshi):
    if Cek_Oshi_Ada(nama_Oshi):
        details = oshis[nama_Oshi]
        print(f"{nama_Oshi} (Gen {details['Gen']}):")
        for prestasi in details['prestasi']:
            print(f"- {prestasi}")
    else:
        print("Oshi tidak ditemukan.")

# Procedure to add Oshi to a user
def Tambah_Oshi(username, nama_Oshi):
    if Cek_Oshi_Ada(nama_Oshi):
        akuns[username]["Oshi"].append(nama_Oshi)
        print(f"{nama_Oshi} telah ditambahkan ke Oshi Kamu!")
    else:
        print("Oshi tidak ditemukan.")

# Procedure to remove Oshi from a user
def Hapus_Oshi(username, nama_Oshi):
    if nama_Oshi in akuns[username]["Oshi"]:
        akuns[username]["Oshi"].remove(nama_Oshi)
        print(f"Oshi {nama_Oshi} telah dihapus dari daftar Kamu.")
    else:
        print("Oshi tidak ditemukan dalam daftar.")

# Fungsi Untuk Menemukan Oshi
def Cek_Oshi(oshi_list, target_oshi):
    if not oshi_list:
        return False
    if oshi_list[0] == target_oshi:
        return True
    return Cek_Oshi(oshi_list[1:], target_oshi)
        ...
        if Username == "Admin" and Password == "Admin123":  # Admin login
            while True:
                ...
                if adminOpsi == "1":
                    ...
                    for user, data in akuns.items():
                        print(f"Username: {user} List Oshi: {', '.join(data['Oshi']) if data['Oshi'] else 'Belum ada Oshi'}")
                    hapus = input("Akun mana yang ingin diubah: ")
                    if hapus in akuns:
                        HapusOshi = input("Masukkan nama member yang ingin dihapus: ")
                        Hapus_Oshi(hapus, HapusOshi)
                    else:
                        print("Akun tidak ditemukan.")
                elif adminOpsi == "5":
